...
public class IsolationBootstrap {
    ...
    public void initialize() {
        ...
        // For development mode, skip database operations
        if (System.getProperty("dev.mode") != null) {
            logger.info("Development mode - skipping database initialization and isolation checks");
            return;
        }
    ...
    public int cleanOrphanedRecords() {
        ...
        try {
            ...
            if (dataCleanupTool != null) {
                ...
                int cleanedCount = 0;
                if (cleanupResults.containsKey("totalOrphanedRecords") && cleanupResults.get("totalOrphanedRecords") instanceof Number) {
                    cleanedCount = ((Number) cleanupResults.get("totalOrphanedRecords")).intValue();
                }
